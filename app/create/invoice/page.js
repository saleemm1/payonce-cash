  ...
  const handleGenerate = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        cn: customerName, d: productDesc, pr: finalPreview, dt: deliveryType
      };
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6 py-10 font-sans">
      <form onSubmit={handleGenerate} className="w-full max-w-md bg-[#18181b] p-8 rounded-2xl border border-white/10 shadow-2xl space-y-4">
        <h1 className="text-2xl font-bold mb-2 text-center text-green-500 uppercase italic font-black">Digital Invoice</h1>
        
        <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500 transition-all" placeholder="Invoice Title / #ID" />
        <input required type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500 transition-all" placeholder="Bill To (Customer Name)" />
        <textarea required value={productDesc} onChange={(e) => setProductDesc(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500 h-20 text-sm" placeholder="Product Description & Details..."></textarea>

        <div className="flex bg-zinc-900 p-1 rounded-lg border border-zinc-700 gap-1">
          <button type="button" onClick={() => setDeliveryType('file')} className={`flex-1 py-2 text-[9px] font-black uppercase rounded-md transition-all ${deliveryType === 'file' ? 'bg-green-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white'}`}>Upload File</button>
        ...
        {deliveryType === 'file' ? (
          ...
          <input required type="text" value={linkOrText} onChange={(e)=>setLinkOrText(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500 text-sm" placeholder="Paste Delivery Link or Secure Info" />
        )}

        <div className="p-3 bg-zinc-800/30 rounded-lg border border-white/5">
          <label className="text-[10px] text-zinc-400 mb-2 block uppercase text-center font-black">Invoice Preview (Optional PDF/IMG)</label>
          <input type="file" accept=".pdf,image/*" onChange={(e)=>setPreviewFile(e.target.files[0])} className="w-full text-xs text-zinc-500 cursor-pointer" />
        </div>

        <div className="grid grid-cols-2 gap-2">
          <input required type="text" placeholder="Company Name" onChange={(e)=>setSellerName(e.target.value)} className="p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500" />
        ...
        <div className="bg-zinc-900/50 p-4 rounded-xl border border-dashed border-zinc-700 flex items-center justify-between">
          <div><h3 className="text-sm font-bold uppercase italic text-white">Private Invoice</h3><p className="text-[10px] text-zinc-500">This bill is generated for one person only</p></div>
        ...
        <button type="submit" disabled={uploading} className="w-full bg-green-600 hover:bg-green-500 py-4 rounded-xl font-black transition-all uppercase italic text-lg shadow-lg disabled:opacity-50">
          {uploading ? "Creating Invoice..." : "Generate Invoice Link"}
        </button>

        {generatedLink && (
          <div className="mt-4 p-3 bg-black rounded-lg border border-green-500/30 flex gap-2 animate-in slide-in-from-top-2">
