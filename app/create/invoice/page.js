...
export default function InvoiceUploadPage() {
  ...
  const handleGenerate = async (e) => {
    e.preventDefault();
    if (!file) return alert("Upload invoice file");
    ...
    try {
      ...
      const payload = {
        ...
        se: sellerEmail, i: json.ipfsHash, a: enableAffiliate
      };
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6 py-10 font-sans">
      <form onSubmit={handleGenerate} className="w-full max-w-md bg-[#18181b] p-8 rounded-2xl border border-white/10 shadow-2xl space-y-4">
        ...
        <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500" placeholder="Invoice # / Description" />
        
        <div>
          <label className="text-[10px] text-zinc-400 mb-1 block ml-1 uppercase font-bold text-center">Upload Invoice File (.pdf, .jpg, .png)</label>
          <input required type="file" accept=".pdf,image/*" onChange={(e)=>setFile(e.target.files[0])} className="w-full p-2 bg-zinc-900 border border-zinc-700 rounded-lg text-sm text-gray-300 file:bg-green-600 cursor-pointer" />
        </div>
        ...
        <div className="bg-zinc-900/50 p-4 rounded-xl border border-dashed border-zinc-700 flex items-center justify-between">
          <div><h3 className="text-sm font-bold uppercase italic">Viral Mode</h3><p className="text-[10px] text-zinc-500">Enable 10% referral bonus</p></div>
        ...
        <button type="submit" disabled={uploading} className="w-full bg-green-600 hover:bg-green-500 py-4 rounded-xl font-black transition-all uppercase italic text-lg shadow-lg disabled:opacity-50">
          {uploading ? "Uploading Invoice..." : "Generate Invoice Link"}
        </button>
