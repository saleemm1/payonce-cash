...
export default function MiniCourseUploadPage() {
  ...
  const handleGenerate = () => {
    ...
    const link = `${window.location.origin}/unlock/mini-course?name=${encodeURIComponent(productName)}&price=${price}&wallet=${wallet}&aff=${enableAffiliate}`;
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-md bg-[#18181b] p-8 rounded-2xl border border-white/10 shadow-2xl">
        <h1 className="text-2xl font-bold mb-6 text-center text-green-500">Upload Mini Course</h1>
        <div className="space-y-4">
          <input type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500" placeholder="Course Name" />
          <div>
            <input type="file" accept=".zip,.rar" className="w-full p-2 bg-zinc-900 border border-zinc-700 rounded-lg text-sm text-gray-300 file:bg-green-600 file:text-white file:border-0 file:px-3 file:py-1 file:rounded" />
            <p className="text-[10px] text-zinc-500 mt-1 ml-1">Allowed formats: ZIP or RAR only</p>
          </div>
