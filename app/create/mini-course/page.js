...
export default function CourseUploadPage() {
  ...
  const [usdPrice, setUsdPrice] = useState(49.99);
  ...
  const handleGenerate = async (e) => {
    e.preventDefault();
    if (!file) return alert("Select course file");
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6 py-10 font-sans">
      <form onSubmit={handleGenerate} className="w-full max-w-md bg-[#18181b] p-8 rounded-2xl border border-white/10 shadow-2xl space-y-4">
        <h1 className="text-2xl font-bold mb-2 text-center text-green-500 uppercase italic font-black">Create Mini-Course</h1>
        <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500 transition-all placeholder:text-zinc-600" placeholder="Course Name" />
        
        <div>
          <label className="text-[10px] text-zinc-400 mb-1 block ml-1 uppercase font-bold">Course Content (All Formats Allowed)</label>
        ...
        <div className="grid grid-cols-2 gap-2">
          <input required type="text" placeholder="Instructor" onChange={(e)=>setSellerName(e.target.value)} className="p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500 transition-all placeholder:text-zinc-600" />
        ...
        <div className="p-3 bg-zinc-800/30 rounded-lg border border-white/5">
          <label className="text-[10px] text-zinc-400 mb-2 block uppercase text-center font-black">Intro Video or Image Preview (Optional)</label>
        ...
        <div className="bg-zinc-900/50 p-4 rounded-xl border border-dashed border-zinc-700 flex items-center justify-between">
          <div><h3 className="text-sm font-bold uppercase italic text-white">Viral Mode</h3><p className="text-[10px] text-zinc-500">10% commission enabled</p></div>
        ...
        <button type="submit" disabled={uploading} className="w-full bg-green-600 hover:bg-green-500 py-4 rounded-xl font-black transition-all uppercase italic text-lg shadow-xl disabled:opacity-50">
          {uploading ? "Processing Course..." : "Generate Course Link"}
        </button>
