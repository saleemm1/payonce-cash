...
const translations = {
  en: {
    title: "Create Mini-Course",
    courseName: "Course Name",
    uploadLabel: "Course Content (.zip, .rar, .pdf)",
    instructor: "Instructor",
    email: "Support Email",
    previewLabel: "Intro Video or Image Preview (Optional)",
    orUrl: "Or Preview URL",
    rate: "BCH Rate:",
  ...
  ar: {
    ...
    uploadLabel: "محتوى الدورة (.zip, .rar, .pdf)",
    instructor: "المحاضر",
    email: "بريد الدعم",
    previewLabel: "فيديو تعريفي أو صورة (اختياري)",
    orUrl: "أو رابط المعاينة",
    rate: "سعر BCH:",
    ...
    generate: "إنشاء رابط الدورة",
    processing: "جاري معالجة الدورة...",
    copy: "نسخ",
    done: "✅",
    supply: "حد المخزون",
  ...
  zh: {
    title: "创建迷你课程",
    courseName: "课程名称",
    uploadLabel: "课程内容 (.zip, .rar, .pdf)",
    instructor: "讲师",
    email: "支持邮箱",
    previewLabel: "介绍视频或图片预览 (可选)",
    orUrl: "或预览链接",
    rate: "BCH 汇率:",
    wallet: "BCH 钱包地址",
    promo: "优惠码",
  ...
  return (
    <div dir={dir} className={`min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-4 py-12 font-sans relative overflow-hidden ${lang === 'ar' ? 'font-arabic' : ''}`}>
      ...
      <form onSubmit={handleGenerate} className="relative z-10 w-full max-w-lg bg-[#18181b]/80 backdrop-blur-2xl p-8 rounded-3xl border border-white/5 shadow-2xl shadow-black/50 space-y-6 transform transition-all hover:border-white/10">
        ...
        <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className={inputBaseStyles} placeholder={t.courseName} />
        ...
        <div className="bg-zinc-900/30 p-4 rounded-xl border border-dashed border-zinc-700 group hover:border-green-500/50 transition-colors duration-300">
          ...
          <input required type="file" accept=".zip,.rar,.pdf,application/zip,application/x-rar-compressed,application/pdf" onChange={(e)=>setFile(e.target.files[0])} className="w-full text-sm text-gray-300 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-green-600 file:text-white hover:file:bg-green-500 file:transition-colors cursor-pointer" />
        </div>

        <div className="grid grid-cols-2 gap-3">
          <input required type="text" placeholder={t.instructor} onChange={(e)=>setSellerName(e.target.value)} className={inputBaseStyles} />
        ...
        <div className="p-4 bg-zinc-800/20 rounded-xl border border-white/5 space-y-3">
          <label className="text-[10px] text-zinc-400 block uppercase text-center font-black tracking-widest opacity-70">{t.previewLabel}</label>
