...
export default function MiniCourseUploadPage() {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6 py-10">
      <form onSubmit={handleGenerate} className="w-full max-w-md bg-[#18181b] p-8 rounded-2xl border border-white/10 shadow-2xl space-y-4">
        <h1 className="text-2xl font-bold mb-2 text-center text-green-500 italic uppercase">Upload Mini Course</h1>
        <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500" placeholder="Course Topic/Title" />
        <div>
          <label className="text-[10px] text-zinc-400 mb-1 block ml-1">Course Content (.zip, .rar only)</label>
          <input required type="file" accept=".zip,.rar" className="w-full p-2 bg-zinc-900 border border-zinc-700 rounded-lg text-sm text-gray-300 file:bg-green-600 mb-2" />
        </div>
        ...
        <div className="p-3 bg-zinc-800/30 rounded-lg border border-white/5">
          <label className="text-[10px] text-zinc-400 mb-2 block uppercase font-bold tracking-widest text-center">Video Preview</label>
          <input type="file" accept="video/*" className="w-full text-xs text-zinc-500 mb-2" />
          <input type="url" placeholder="Or Trailer URL" onChange={(e)=>setPreviewLink(e.target.value)} className="w-full p-2 bg-zinc-900 border border-zinc-700 rounded-lg text-xs outline-none focus:border-green-500" />
        </div>
        <div className="relative">
          <label className="text-[10px] text-zinc-400 mb-1 block italic font-bold tracking-tighter">Est. Price: {bchPreview} BCH</label>
        ...
        <div className="bg-zinc-900/50 p-4 rounded-xl border border-dashed border-zinc-700 flex items-center justify-between">
          <div><h3 className="text-sm font-bold italic uppercase tracking-widest">Viral Mode</h3><p className="text-[10px] text-zinc-500 tracking-tighter">10% referral rewards</p></div>
        ...
        <button type="submit" className="w-full bg-green-600 hover:bg-green-500 py-4 rounded-xl font-bold transition-all uppercase italic text-lg">Generate Course Link</button>
        {generatedLink && (
