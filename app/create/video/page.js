  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6 py-10">
      <form onSubmit={handleGenerate} className="w-full max-w-md bg-[#18181b] p-8 rounded-2xl border border-white/10 shadow-2xl space-y-4">
        <h1 className="text-2xl font-bold mb-2 text-center text-green-500 uppercase italic">Upload Premium Video</h1>
        ...
        <div>
          <label className="text-[10px] text-zinc-400 mb-1 block ml-1 uppercase font-bold tracking-tighter">Master Video File (.mp4, .mov, .avi)</label>
          <input required type="file" accept="video/mp4,video/quicktime,video/x-msvideo" className="w-full p-2 bg-zinc-900 border border-zinc-700 rounded-lg text-sm text-gray-300 file:bg-green-600 mb-2" />
        </div>
        ...
        <div className="p-4 bg-zinc-800/30 rounded-lg border border-white/5">
          <label className="text-[10px] text-zinc-400 mb-2 block uppercase text-center font-black">Trailer / Preview (Optional)</label>
          <input type="file" accept="video/*,image/*" className="w-full text-xs text-zinc-500 mb-2" />
        ...
        <div className="relative">
          <label className="text-[10px] text-zinc-400 mb-1 block italic font-bold">Estimated Rate: {bchPreview} BCH</label>
        ...
        <div className="bg-zinc-900/50 p-4 rounded-xl border border-dashed border-zinc-700 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-bold uppercase italic tracking-widest text-white">Viral Mode</h3>
            <p className="text-[10px] text-zinc-500">Enable 10% referral commission</p>
          </div>
        ...
        <button type="submit" className="w-full bg-green-600 hover:bg-green-500 py-4 rounded-xl font-black transition-all uppercase italic text-lg shadow-lg shadow-green-900/20">Generate Video Link</button>

        {generatedLink && (
          <div className="mt-4 p-3 bg-black rounded-lg border border-green-500/30 flex gap-2 animate-pulse">
