  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6 py-10">
      <form onSubmit={handleGenerate} className="w-full max-w-md bg-[#18181b] p-8 rounded-2xl border border-white/10 shadow-2xl space-y-4">
        ...
        <div>
          <label className="text-[10px] text-zinc-400 mb-1 block ml-1 uppercase font-bold">Master Video (.mp4, .mov, .avi)</label>
          <input required type="file" accept="video/*" className="w-full p-2 bg-zinc-900 border border-zinc-700 rounded-lg text-sm text-gray-300 file:bg-green-600" />
        </div>
        ...
        <div className="p-3 bg-zinc-800/30 rounded-lg border border-white/5">
          <label className="text-[10px] text-zinc-400 mb-2 block uppercase text-center font-black">Trailer / Preview</label>
        ...
        <div className="bg-zinc-900/50 p-4 rounded-xl border border-dashed border-zinc-700 flex items-center justify-between">
          <div><h3 className="text-sm font-bold uppercase italic">Viral Mode</h3><p className="text-[10px] text-zinc-500">10% affiliate commission</p></div>
        ...
        <button type="submit" className="w-full bg-green-600 hover:bg-green-500 py-4 rounded-xl font-black transition-all uppercase italic text-lg">Generate Video Link</button>
      </form>
    </div>
  );
}
