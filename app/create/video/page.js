...
const translations = {
  en: {
    title: "Upload Premium Video",
    videoName: "Video Title",
    uploadLabel: "Master Video (.mp4, .mov, .avi)",
    creator: "Creator Name",
    email: "Support Email",
    previewLabel: "Trailer / Preview (File or URL) (Optional)",
  ...
  ar: {
    ...
    videoName: "عنوان الفيديو",
    uploadLabel: "ملف الفيديو الرئيسي (.mp4, .mov, .avi)",
    creator: "اسم المنشئ",
    email: "بريد الدعم",
    previewLabel: "إعلان / معاينة (ملف أو رابط) (اختياري)",
  ...
  zh: {
    title: "上传优质视频",
    videoName: "视频标题",
    uploadLabel: "主视频文件 (.mp4, .mov, .avi)",
    creator: "创作者姓名",
    email: "支持邮箱",
    previewLabel: "预告片 / 预览 (文件或链接) (可选)",
    orUrl: "或预览链接",
    rate: "BCH 汇率:",
    wallet: "BCH 钱包地址",
    promo: "优惠码",
  ...
  return (
    <div dir={dir} className={`min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-4 py-12 font-sans relative overflow-hidden ${lang === 'ar' ? 'font-arabic' : ''}`}>
      ...
      <form onSubmit={handleGenerate} className="relative z-10 w-full max-w-lg bg-[#18181b]/80 backdrop-blur-2xl p-8 rounded-3xl border border-white/5 shadow-2xl shadow-black/50 space-y-6 transform transition-all hover:border-white/10">
        ...
        <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className={inputBaseStyles} placeholder={t.videoName} />
        ...
        <div className="grid grid-cols-2 gap-3">
          <input required type="text" placeholder={t.creator} onChange={(e)=>setSellerName(e.target.value)} className={inputBaseStyles} />
