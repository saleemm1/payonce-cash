...
export default function GameCardUploadPage() {
  ...
  const handleGenerate = () => {
    ...
    const link = `${window.location.origin}/unlock/game-card?name=${encodeURIComponent(productName)}&price=${price}&wallet=${wallet}&aff=${enableAffiliate}`;
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-md bg-[#18181b] p-8 rounded-2xl border border-white/10 shadow-2xl">
        <h1 className="text-2xl font-bold mb-6 text-center text-green-500">Upload Game Card</h1>
        <div className="space-y-4">
          <input type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500" placeholder="Game Name" />
          <div>
          ...
          <div className="bg-zinc-900/50 p-4 rounded-xl border border-dashed border-zinc-700">
            <div className="flex items-center justify-between">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" checked={enableAffiliate} onChange={(e) => setEnableAffiliate(setEnableAffiliate)} className="sr-only peer" />
