...
export default function CodeUploadPage() {
  ...
  const handleGenerate = async (e) => {
    e.preventDefault();
    if (!file) return alert("Please upload the source code file");
    ...
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6 py-10 font-sans">
      <form onSubmit={handleGenerate} className="w-full max-w-md bg-[#18181b] p-8 rounded-2xl border border-white/10 shadow-2xl space-y-4">
        ...
        <div className="text-center mb-6">
            <h1 className="text-2xl font-bold mb-1 text-green-500 uppercase italic font-black">Sell Source Code</h1>
            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Python • JS • PHP • Java • Solidity</p>
        </div>

        <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500 transition-all" placeholder="Script / Project Name" />
        
        <div>
          <label className="text-[10px] text-zinc-400 mb-1 block ml-1 uppercase font-bold">Source File (.zip, .js, .py, .php, etc)</label>
          <input required type="file" accept=".zip,.rar,.7z,.js,.py,.php,.java,.c,.cpp,.html,.css,.json,.sol" onChange={(e)=>setFile(e.target.files[0])} className="w-full p-2 bg-zinc-900 border border-zinc-700 rounded-lg text-sm text-gray-300 file:bg-green-600 cursor-pointer" />
        </div>

        <div className="grid grid-cols-2 gap-2">
          <input required type="text" placeholder="Dev Name / Handle" onChange={(e)=>setSellerName(e.target.value)} className="p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500 transition-all" />
        ...
        <div className="p-3 bg-zinc-800/30 rounded-lg border border-white/5">
          <label className="text-[10px] text-zinc-400 mb-2 block uppercase text-center font-black">Thumbnail or Screenshot (Optional)</label>
        ...
        <div className="bg-zinc-900/50 p-4 rounded-xl border border-dashed border-zinc-700 flex items-center justify-between group hover:border-green-500/30 transition-colors">
          <div>
            <h3 className="text-sm font-bold uppercase italic text-white group-hover:text-green-500 transition-colors">Viral Mode</h3>
        ...
        <button type="submit" disabled={uploading} className="w-full bg-green-600 hover:bg-green-500 py-4 rounded-xl font-black transition-all uppercase italic text-lg shadow-xl disabled:opacity-50 hover:-translate-y-1">
          {uploading ? "Deploying Code..." : "Generate Code Link"}
        </button>
