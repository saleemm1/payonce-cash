    ...
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-4 py-12 font-sans relative overflow-hidden">
      ...
      <form onSubmit={handleGenerate} className="relative z-10 w-full max-w-lg bg-[#18181b]/80 backdrop-blur-2xl p-8 rounded-3xl border border-white/5 shadow-2xl shadow-black/50 space-y-6 transform transition-all hover:border-white/10">
        <div className="text-center space-y-1">
            <h1 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-green-600 uppercase italic tracking-tighter drop-shadow-sm">Publish Digital Book</h1>
            <div className="h-1 w-20 bg-green-500/50 rounded-full mx-auto"></div>
        </div>
        
        <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className={inputBaseStyles} placeholder="Book Title" />
        ...
        <div className="bg-zinc-900/30 p-4 rounded-xl border border-dashed border-zinc-700 group hover:border-green-500/50 transition-colors duration-300">
          <label className="text-[11px] text-zinc-400 mb-2 block uppercase font-bold tracking-wider group-hover:text-green-400 transition-colors">Main Book File (PDF/EPUB)</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          <input required type="text" placeholder="Author Name" onChange={(e)=>setSellerName(e.target.value)} className={inputBaseStyles} />
        ...
        <div className="p-4 bg-zinc-800/20 rounded-xl border border-white/5 space-y-3">
          ...
          <div className="relative">
             ...
             <input type="url" placeholder="Or Cover Image URL" value={previewLink} onChange={(e)=>setPreviewLink(e.target.value)} className={`${inputBaseStyles} pl-8 py-2 text-xs`} />
          </div>
        </div>
