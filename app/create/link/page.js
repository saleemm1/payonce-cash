...
const translations = {
  en: {
    title: "Sell Secure Link",
    examples: ["Zoom / Google Meet", "Google Forms / Sheets", "Unlisted YouTube", "Telegram Invite"],
    service: "Service Title (e.g. Consultation, Survey Access)",
    secretLabel: "The Secret URL (Hidden)",
    seller: "Seller Name",
    email: "Support Email",
    coverLabel: "Cover Preview (Optional)",
    orUrl: "Or Image URL",
    rate: "BCH Rate:",
    ...
    generate: "Generate PayLink",
    processing: "Encrypting Link...",
    copy: "Copy",
    done: "✅",
    supply: "Supply Limit",
  ...
  ar: {
    title: "بيع رابط آمن",
    examples: ["زوم / جوجل ميت", "نماذج جوجل", "يوتيوب غير مدرج", "دعوة تيليجرام"],
    service: "عنوان الخدمة (استشارة، وصول لاستبيان)",
    secretLabel: "الرابط السري (مخفي)",
    seller: "اسم البائع",
    email: "بريد الدعم",
    coverLabel: "معاينة الغلاف (اختياري)",
    ...
    generate: "إنشاء رابط الدفع",
    processing: "جاري تشفير الرابط...",
    copy: "نسخ",
    done: "✅",
    supply: "حد المخزون",
  ...
  zh: {
    title: "出售安全链接",
    examples: ["Zoom / Google Meet", "Google 表单 / 表格", "不公开 YouTube", "Telegram 邀请"],
    service: "服务标题 (例如：咨询，调查访问)",
    secretLabel: "秘密 URL (隐藏)",
    seller: "卖家名称",
    email: "支持邮箱",
    coverLabel: "封面预览 (可选)",
    orUrl: "或图片链接",
      ...
      <form onSubmit={handleGenerate} className="relative z-10 w-full max-w-lg bg-[#18181b]/80 backdrop-blur-2xl p-8 rounded-3xl border border-white/5 shadow-2xl shadow-black/50 space-y-6 transform transition-all hover:border-white/10">
        ...
        <div className="text-center mb-6">
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-4">
                {t.examples.map((ex, i) => (
                    <span key={i} className="text-[10px] bg-zinc-800/50 border border-zinc-700/50 px-2.5 py-1 rounded-full text-zinc-400">{ex}</span>
                ))}
            </div>
            <div className="h-1 w-20 bg-green-500/50 rounded-full mx-auto mt-4"></div>
        </div>
        
        <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className={inputBaseStyles} placeholder={t.service} />
        
        <div className="relative group">
          <label className="text-[10px] text-zinc-400 mb-2 block uppercase font-bold tracking-wider text-center group-hover:text-green-400 transition-colors">{t.secretLabel}</label>
          <div className="relative">
              <input required type="url" value={secretLink} onChange={(e)=>setSecretLink(e.target.value)} className={`${inputBaseStyles} ${lang === 'ar' ? 'pl-10' : 'pr-10'} font-mono text-sm`} placeholder="https://..." />
              <div className={`absolute ${lang === 'ar' ? 'left-3' : 'right-3'} top-3.5 text-lg text-green-500 animate-pulse`}>🔒</div>
          </div>
        </div>
