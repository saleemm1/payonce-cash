...
export default function FolderUploadPage() {
  ...
  const handleGenerate = async (e) => {
    e.preventDefault();
    if (!file) return alert("Please upload a ZIP archive");
    ...
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6 py-10 font-sans">
      <form onSubmit={handleGenerate} className="w-full max-w-md bg-[#18181b] p-8 rounded-2xl border border-white/10 shadow-2xl space-y-4">
        ...
        <div className="text-center mb-6">
            <h1 className="text-2xl font-bold mb-1 text-green-500 uppercase italic font-black">Sell Secure Folder</h1>
            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Project Archives • Asset Packs • Bulk Data</p>
        </div>

        <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500 transition-all" placeholder="Folder / Archive Name" />
        
        <div>
          <label className="text-[10px] text-zinc-400 mb-1 block ml-1 uppercase font-bold">Archive File (.zip, .rar, .7z)</label>
          <input required type="file" accept=".zip,.rar,.7z,.tar,.gz" onChange={(e)=>setFile(e.target.files[0])} className="w-full p-2 bg-zinc-900 border border-zinc-700 rounded-lg text-sm text-gray-300 file:bg-green-600 cursor-pointer" />
        </div>
        ...
        <button type="submit" disabled={uploading} className="w-full bg-green-600 hover:bg-green-500 py-4 rounded-xl font-black transition-all uppercase italic text-lg shadow-xl disabled:opacity-50 hover:-translate-y-1">
          {uploading ? "Compressing & Encrypting..." : "Secure Folder Link"}
        </button>
