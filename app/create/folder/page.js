...
const translations = {
  en: {
    title: "Sell Secure Folder",
    sub: "Project Archives • Asset Packs • Bulk Data",
    folderName: "Folder / Archive Name",
    uploadLabel: "Archive File (.zip, .rar, .7z)",
    seller: "Seller Name",
    email: "Support Email",
    ...
    processing: "Compressing & Encrypting...",
    copy: "Copy",
    done: "✅"
  },
  ar: {
    title: "بيع مجلد آمن",
    sub: "أرشيف المشاريع • حزم الأصول • بيانات ضخمة",
    folderName: "اسم المجلد / الأرشيف",
    uploadLabel: "ملف الأرشيف (.zip, .rar, .7z)",
    seller: "اسم البائع",
    email: "بريد الدعم",
    ...
    generate: "تأمين رابط المجلد",
    processing: "جاري الضغط والتشفير...",
    copy: "نسخ",
    done: "✅"
  },
  zh: {
  ...
  return (
    <div dir={dir} className={`min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-4 py-12 font-sans relative overflow-hidden ${lang === 'ar' ? 'font-arabic' : ''}`}>
      ...
      <form onSubmit={handleGenerate} className="relative z-10 w-full max-w-lg bg-[#18181b]/80 backdrop-blur-2xl p-8 rounded-3xl border border-white/5 shadow-2xl shadow-black/50 space-y-6 transform transition-all hover:border-white/10">
        ...
        <input required type="text" value={productName} onChange={(e) => setProductName(e.target.value)} className={inputBaseStyles} placeholder={t.folderName} />
        ...
        <div className="bg-zinc-900/30 p-4 rounded-xl border border-dashed border-zinc-700 group hover:border-green-500/50 transition-colors duration-300">
          ...
          <input required type="file" accept=".zip,.rar,.7z,.tar,.gz" onChange={(e)=>setFile(e.target.files[0])} className="w-full text-sm text-gray-300 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-green-600 file:text-white hover:file:bg-green-500 file:transition-colors cursor-pointer" />
        </div>
