...
export default function AppActivateCardPage() {
  ...
  const handleGenerate = (e) => {
    e.preventDefault();
    if (!wallet || !productName || !sellerEmail || !priceUSD) return alert('Please fill required fields!');
    const link = `${window.location.origin}/unlock?name=${encodeURIComponent(productName)}&usd=${priceUSD}&wallet=${wallet}&seller=${encodeURIComponent(sellerName)}&email=${encodeURIComponent(sellerEmail)}&preview=${encodeURIComponent(previewLink)}&aff=${enableAffiliate}`;
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex flex-col items-center justify-center px-6 py-10">
      <form onSubmit={handleGenerate} className="w-full max-w-md bg-[#18181b] p-8 rounded-2xl border border-white/10 shadow-2xl space-y-4">
        ...
        <input required type="file" className="w-full p-2 bg-zinc-900 border border-zinc-700 rounded-lg text-sm text-gray-300" />
        ...
        <input type="url" placeholder="Preview Link (Optional)" onChange={(e)=>setPreviewLink(e.target.value)} className="w-full p-3 bg-zinc-900 border border-zinc-700 rounded-lg text-white outline-none focus:border-green-500" />
        <div className="bg-zinc-900 p-3 rounded-lg border border-zinc-700">
          <label className="text-[10px] text-zinc-400 mb-2 block">Set Price in USD (BCH will update live)</label>
          <input required type="number" step="0.01" value={priceUSD} onChange={(e) => setPriceUSD(e.target.value)} className="w-full p-2 bg-zinc-800 border border-zinc-600 rounded text-white text-sm outline-none focus:border-green-500" placeholder="Price in USD ($)" />
        </div>
        ...
        <button type="submit" className="w-full bg-green-600 hover:bg-green-500 py-3 rounded-xl font-bold transition-all">GENERATE LIVE PRICE LINK</button>
        {generatedLink && (
