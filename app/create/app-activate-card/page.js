      ...
      <form onSubmit={handleGenerate} className="relative z-10 w-full max-w-lg bg-[#18181b]/80 backdrop-blur-2xl p-8 rounded-3xl border border-white/5 shadow-2xl shadow-black/50 space-y-6 transform transition-all hover:border-white/10">
        ...
        {deliveryType === 'file' ? (
          <div className="bg-zinc-900/30 p-4 rounded-xl border border-dashed border-zinc-700 group hover:border-green-500/50 transition-colors duration-300">
            <label className="text-[11px] text-zinc-400 mb-2 block uppercase font-bold tracking-wider text-center group-hover:text-green-400 transition-colors">Upload License/Key File</label>
          ...
          <textarea required value={textCode} onChange={(e)=>setTextCode(e.target.value)} placeholder="Paste activation keys here..." className={`${inputBaseStyles} h-32 resize-none`}></textarea>
        )}

        <div className="p-4 bg-zinc-800/20 rounded-xl border border-white/5 space-y-3">
          <label className="text-[10px] text-zinc-400 block uppercase text-center font-black tracking-widest opacity-70">App Icon/Preview (File or URL) (Optional)</label>
          ...
          <div className="relative">
             ...
             <input type="url" placeholder="Or Icon URL" value={previewLink} onChange={(e)=>setPreviewLink(e.target.value)} className={`${inputBaseStyles} pl-8 py-2 text-xs`} />
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <input required type="text" placeholder="Dev Name" onChange={(e)=>setSellerName(e.target.value)} className={inputBaseStyles} />
