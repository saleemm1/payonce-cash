...
const PLATFORM_WALLET = "bitcoincash:qp111";
          ...
          <div className="bg-[#121214] border border-white/5 rounded-3xl p-6 shadow-2xl space-y-6 relative overflow-hidden group">
            ...
            <div className="space-y-4">
              <div className="flex gap-4 items-center">
                <img src={profile.avatar} alt="Avatar" className="w-16 h-16 rounded-full border border-white/10 bg-black" />
                <div className="flex-1">
                  <label className="text-[10px] uppercase font-bold text-zinc-500 block mb-1">Avatar Seed (DiceBear)</label>
                  <input type="text" value={profile.avatar.split('seed=')[1] || ''} onChange={(e) => setProfile({...profile, avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${e.target.value}`})} className="w-full bg-black/50 border border-white/10 rounded-xl p-3 text-sm text-white outline-none focus:border-green-500 transition-colors" placeholder="Type any word..." />
                </div>
              </div>
        ...
        <div className="lg:col-span-5 relative flex justify-center lg:justify-end">
          <div className="sticky top-12 w-[320px] h-[650px] bg-black border-[8px] border-zinc-800 rounded-[3rem] shadow-2xl overflow-hidden flex flex-col items-center relative z-10">
            ...
            <div className="w-full h-full bg-gradient-to-b from-zinc-900 to-black p-6 overflow-y-auto custom-scrollbar flex flex-col items-center text-center pt-16">
              <img src={profile.avatar} alt="Avatar" className="w-24 h-24 rounded-full border-4 border-black shadow-xl mb-4 transition-transform hover:scale-105" />
              ...
              <div className="w-full space-y-3">
                {links.map(link => (
                  <div key={link.id} className="w-full bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl p-4 text-sm font-bold transition-all cursor-pointer hover:scale-[1.02] flex items-center justify-between">
                    ...
                    <span className="text-[10px] bg-green-500/20 text-green-500 px-2 py-1 rounded-md">BCH</span>
                  </div>
                ))}
              </div>
